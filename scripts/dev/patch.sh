...
normalize_file() {
  ...
  if [[ "${FILE}" == config/patches/* ]]; then
    FILE="../${FILE}"
  else
    FILE="../config/patches/${FILE}"
  fi
}
...
. "$(dirname "${BASH_SOURCE[0]}")/../lib/utils.sh"
...
if [[ "${FILE}" != "../config/patches/helper/settings.patch" ]]; then
  git apply --reject "../config/patches/helper/settings.patch"
