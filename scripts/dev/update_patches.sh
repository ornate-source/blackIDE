...
for FILE in ../config/patches/*.patch; do
  ADDITIONAL_FILES=()
...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  for FILE in ../config/patches/insider/*.patch; do
    check_file "${FILE}"
  done
fi

for ARCH in alpine linux osx windows; do
  for FILE in "../config/patches/${ARCH}/"*.patch; do
    ADDITIONAL_FILES=()
    BASENAME=$(basename "${FILE}")
    DIRNAME=$(dirname "${FILE}")
  ...
  for TARGET in client reh; do
    for FILE in "../config/patches/${ARCH}/${TARGET}/"*.patch; do
      check_file "${FILE}"
    done
