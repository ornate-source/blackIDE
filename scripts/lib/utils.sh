...
APP_NAME="${APP_NAME:-Black IDE}"
APP_NAME_LC="$( echo "${APP_NAME}" | awk '{print tolower($0)}' | tr ' ' '-' )"
ASSETS_REPOSITORY="${ASSETS_REPOSITORY:-ornate-source/blackIDE}"
BINARY_NAME="${BINARY_NAME:-black-ide}"
GH_REPO_PATH="${GH_REPO_PATH:-ornate-source/blackIDE}"
ORG_NAME="${ORG_NAME:-ornate-source}"
      ...
      remove)
        jq -r '.paths[]' <<< "${ENTRY}" | while IFS= read -r ENTRY_PATH; do
          ...
          if [[ -e "${ENTRY_PATH}" ]]; then
            ...
            echo "Already not found (skipped): ${ENTRY_PATH}"
          fi
        done
      ;;
    esac
