...
generateJson() {
  ...
  if [[ ! -f "${PROJECT_ROOT}/assets/${ASSET_NAME}" ]]; then
    echo "Downloading asset '${ASSET_NAME}'"
    gh release download --repo "${ASSETS_REPOSITORY}" "${RELEASE_VERSION}" --dir "${PROJECT_ROOT}/assets" --pattern "${ASSET_NAME}*"
  fi

  sha1hash=$( awk '{ print $1 }' "${PROJECT_ROOT}/assets/${ASSET_NAME}.sha1" )
  sha256hash=$( awk '{ print $1 }' "${PROJECT_ROOT}/assets/${ASSET_NAME}.sha256" )
...
updateLatestVersion() {
  ...
  if [[ -f "${PROJECT_ROOT}/${REPOSITORY_NAME}/${VERSION_PATH}/latest.json" ]]; then
    CURRENT_VERSION=$( jq -r '.name' "${PROJECT_ROOT}/${REPOSITORY_NAME}/${VERSION_PATH}/latest.json" )
  ...
  mkdir -p "${PROJECT_ROOT}/${REPOSITORY_NAME}/${VERSION_PATH}"

  generateJson

  echo "${JSON_DATA}" > "${PROJECT_ROOT}/${REPOSITORY_NAME}/${VERSION_PATH}/latest.json"
...
cd "${PROJECT_ROOT}"
if ! git clone "https://${GH_HOST}/${VERSIONS_REPOSITORY}.git"; then
  echo "WARNING: Could not clone versions repository at ${VERSIONS_REPOSITORY}. Bypassing versions update."
  exit 0
fi
cd "${PROJECT_ROOT}/${REPOSITORY_NAME}" || { echo "'${REPOSITORY_NAME}' dir not found"; exit 0; }
...
cd "${PROJECT_ROOT}/${REPOSITORY_NAME}" || { echo "'${REPOSITORY_NAME}' dir not found"; exit 1; }
