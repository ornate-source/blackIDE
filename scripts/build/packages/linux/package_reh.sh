...
if [[ -d "../config/patches/linux/reh/" ]]; then
  for file in "../config/patches/linux/reh/"*.patch; do
    if [[ -f "${file}" ]]; then
      apply_patch "${file}"
    fi
  done
...
if [[ -d "../config/patches/linux/reh/${VSCODE_ARCH}/" ]]; then
  for file in "../config/patches/linux/reh/${VSCODE_ARCH}/"*.patch; do
    if [[ -f "${file}" ]]; then
      apply_patch "${file}"
    fi
  done
...
if [[ "${SHOULD_BUILD_REH}" != "no" ]]; then
  ...
  #   SEARCH_PATH="../vscode-reh-${VSCODE_PLATFORM}-${VSCODE_ARCH}" ../scripts/build/packages/linux/verify_cxx11_requirements.sh
  # fi

  pushd "../vscode-reh-${VSCODE_PLATFORM}-${VSCODE_ARCH}"

  if [[ -f "../scripts/build/packages/linux/${VSCODE_ARCH}/ripgrep.sh" ]]; then
    bash "../scripts/build/packages/linux/${VSCODE_ARCH}/ripgrep.sh" "node_modules"
  fi
