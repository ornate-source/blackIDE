...
if [[ -f "../scripts/build/packages/linux/${VSCODE_ARCH}/electron.sh" ]]; then
  # add newline at the end of the file
  echo "" >> build/checksums/electron.txt

  if [[ -f "../scripts/build/packages/linux/${VSCODE_ARCH}/electron.sha256sums" ]]; then
    cat "../scripts/build/packages/linux/${VSCODE_ARCH}/electron.sha256sums" >> build/checksums/electron.txt
  fi

  # shellcheck disable=SC1090
  source "../scripts/build/packages/linux/${VSCODE_ARCH}/electron.sh"
...
if [[ -d "../config/patches/linux/client/" ]]; then
  for file in "../config/patches/linux/client/"*.patch; do
    if [[ -f "${file}" ]]; then
      apply_patch "${file}"
    fi
  done
...
if [[ -f "../scripts/build/packages/linux/${VSCODE_ARCH}/ripgrep.sh" ]]; then
  bash "../scripts/build/packages/linux/${VSCODE_ARCH}/ripgrep.sh" "../VSCode-linux-${VSCODE_ARCH}/resources/app/node_modules"
fi
