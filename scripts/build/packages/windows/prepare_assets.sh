...
# . ../scripts/build/packages/windows/appx/build.sh
...
if [[ "${VSCODE_ARCH}" == "ia32" || "${VSCODE_ARCH}" == "x64" ]]; then
  if [[ "${SHOULD_BUILD_MSI}" != "no" ]]; then
    . ../scripts/build/packages/windows/msi/build.sh
  fi

  if [[ "${SHOULD_BUILD_MSI_NOUP}" != "no" ]]; then
    . ../scripts/build/packages/windows/msi/build-updates-disabled.sh
  fi
fi
...
if [[ "${VSCODE_ARCH}" == "ia32" || "${VSCODE_ARCH}" == "x64" ]]; then
  if [[ "${SHOULD_BUILD_MSI}" != "no" ]]; then
    echo "Moving MSI"
    mv "scripts\\build\\packages\\windows\\msi\\releasedir\\${APP_NAME}-${VSCODE_ARCH}-${RELEASE_VERSION}.msi" assets/
  fi

  if [[ "${SHOULD_BUILD_MSI_NOUP}" != "no" ]]; then
    echo "Moving MSI with disabled updates"
    mv "scripts\\build\\packages\\windows\\msi\\releasedir\\${APP_NAME}-${VSCODE_ARCH}-updates-disabled-${RELEASE_VERSION}.msi" assets/
  fi
fi
