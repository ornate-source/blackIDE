...
[[ -z "${SRC_PREFIX}" ]] && SRC_PREFIX="${PROJECT_ROOT}/"
[[ -z "${VSCODE_PREFIX}" ]] && VSCODE_PREFIX="${PROJECT_ROOT}/config/"
...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    ...
    if [[ "$1" == "no-template" ]]; then
      rsvg-convert -w 1024 -h 1024 "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl.svg" -o "code_1024.png"
    else
      rsvg-convert -w 655 -h 655 "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl.svg" -o "code_logo.png"
...
build_darwin_types() { # {{{
  if [[ "$1" == "no-border" ]]; then
    rsvg-convert -w 128 -h 128 "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl.svg" -o "code_logo.png"
  else
    rsvg-convert -w 128 -h 128 "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl_w80_b8.svg" -o "code_logo.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    ...
    cp "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_clt.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  mkdir -p "${SRC_PREFIX}src/${QUALITY}/resources/win32" "${SRC_PREFIX}scripts/build/packages/windows/msi/resources/${QUALITY}"

  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "${PROJECT_ROOT}/config/icons/${QUALITY}/codium_cnl.svg" -o "code_logo.png"
  ...
  build_windows_type "${SRC_PREFIX}scripts/build/packages/windows/msi/resources/${QUALITY}/wix-banner.bmp" "493x58" "xc:white" "50" "+438+6"
  build_windows_type "${SRC_PREFIX}scripts/build/packages/windows/msi/resources/${QUALITY}/wix-dialog.bmp" "493x312" "xc:white" "120" "+22+152"
