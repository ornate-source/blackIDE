...
if [[ "${SHOULD_DEPLOY}" == "no" ]]; then
  ...
  if [[ "${LATEST_VERSION}" == "null" || -z "${LATEST_VERSION}" || "${GITHUB_RESPONSE}" =~ "Not Found" ]]; then
    echo "No latest release found in the repository. Assuming first release."
    ...
    if [[ "${SHOULD_BUILD}" != "yes" ]]; then
      echo "Same upstream version, creating new release with version: ${RELEASE_VERSION}"
      export SHOULD_BUILD="yes"
      ASSETS="null"
    else
      ASSETS="null"
...
if [[ "${CHECK_ASSETS}" == "no" ]]; then
  ...
  # macos
    ...
    . "${PROJECT_ROOT}/scripts/build/packages/osx/check_tags.sh"
  elif [[ "${OS_NAME}" == "windows" ]]; then
    . "${PROJECT_ROOT}/scripts/build/packages/windows/check_tags.sh"
  else
    if [[ "${OS_NAME}" == "linux" ]]; then
      . "${PROJECT_ROOT}/scripts/build/packages/linux/check_tags.sh"
    fi

    if [[ "${OS_NAME}" == "alpine" ]] || [[ "${OS_NAME}" == "linux" && "${CHECK_ALL}" == "yes" ]]; then
      . "${PROJECT_ROOT}/scripts/build/packages/alpine/check_tags.sh"
    fi
  fi
else
  if [[ "${IS_SPEARHEAD}" == "yes" ]]; then
