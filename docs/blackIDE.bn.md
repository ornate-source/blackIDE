...
- স্টোরেজ কোনো SQL ডেটাবেস **নয়** — ডিস্কে দুটি ফাইল ব্যবহৃত হয়: চাঙ্ক মেটাডেটার জন্য `codebase-index.json` এবং ভেক্টরের জন্য `vectors.bin` (একটি ফ্ল্যাট `Float32` বাইনারি স্টোর)।
- এম্বেডিং **অপশনাল এবং প্রোভাইডার-নির্ভর** — OpenAI `text-embedding-3-small` অথবা Ollama `nomic-embed-text`। কোনো প্রোভাইডার কনফিগার করা না থাকলে রিট্রিভাল BM25 কীওয়ার্ড র‍্যাঙ্কিং-এ ফিরে যায়।
- **লাইন-উইন্ডো চাঙ্কিং**: ফাইলগুলো ১০ লাইন ওভারল্যাপ সহ নির্দিষ্ট ৫০ লাইনের উইন্ডোতে ভাগ করা হয় (`core/codebase-index.ts`-এ `CHUNK_LINES` / `CHUNK_OVERLAP`)। চাঙ্কের সীমানা সিম্বল-অ্যাওয়্যার **নয়**, তাই একটি চাঙ্ক ফাংশনের মাঝখান থেকে শুরু বা শেষ হতে পারে। ক্লাস/ফাংশন সীমানা অনুযায়ী সিম্বল-অ্যাওয়্যার চাঙ্কিং পরিকল্পনায় রয়েছে — দেখুন [`notes/enhancement.md`](notes/enhancement.md) (E2)।
- র‍্যাঙ্কিং-এ এম্বেডিং সিমিলারিটি এবং BM25 একত্রিত করা হয় Reciprocal Rank Fusion (RRF)-এর মাধ্যমে।
- এজেন্ট `codebase_search` টুলের মাধ্যমে এই ইনডেক্স ব্যবহার করে।
