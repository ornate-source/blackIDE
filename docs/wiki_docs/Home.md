...
3. **Surgical Undo System**: The checkpoint manager calculates atomic git-style diffs for all AI edits, letting you revert changes easily.
4. **Isolated Parallel Subagents**: Spawn multiple subagents safely using automated Git worktrees to prevent index locks or conflicts.
5. **Inline Editor Chat (`Cmd+I`)**: Fast, editor-native inline refactoring with line offset tracking and visual change highlights.
...
* **Developer Guides**
  * [[Building from Source|How-to-Build]] - Compiling and packaging Black IDE locally.
