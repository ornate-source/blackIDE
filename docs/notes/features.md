...
| 108 | **The Office is reachable without opening anything** | ✅ | 🟢 | A `◆ Office` status bar entry that reads `3▸ 1!` — running, and waiting on you — sourced from the governor and `inboxCounts`, fed by the lane's own poll so it is correct when no panel is open. Plus a **Front Desk** rendering the same floor beside your work — a collapsible pane stacked under the chat in the one **Black Agent** container, so the runs are under the conversation that launched them and watching one does not hide the other — a `✦ Black IDE: Agent Office` command that focuses that pane, and a blocked-run toast that reveals it rather than taking over an editor column. Each segment appears only when it has a number to show: `◆ Office 0▸ 0!` is how a badge gets ignored. `core/office-status.ts`, `core/office-sidebar.ts`, `webview/src/FrontDesk.tsx` |
