...
const openaiToolTurn = sse([
  ...
  { choices: [{ delta: { tool_calls: [{ index: 0, id: 'c1', type: 'function', function: { name: 'read_file', arguments: '{"path":"a.txt"}' } }] } }] },
  '[DONE]',
]);
...
const claudeToolTurn = sse([
  { type: 'content_block_delta', delta: { type: 'text_delta', text: 'Reading now.' } },
  { type: 'content_block_start', index: 1, content_block: { type: 'tool_use', id: 'c1', name: 'read_file', input: { path: 'a.txt' } } },
...
global.fetch = async (url, init) => {
  ...
  } else if (urlStr.includes('/claude')) {
    responseText = claudeToolTurn;
  } else if (urlStr.includes('/gemini')) {
    contentType = 'application/json';
    responseText = geminiToolTurn;
...
async function main() {
  ...
  console.log('--- STARTING BLOCK 13 ---');
  {
    const { agentReducer, initialAgentState, phaseLabel, elapsedMs, pendingReview } =
      require(path.join(__dirname, 'test', 'agent-store.js'));
